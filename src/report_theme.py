...
# ⚠️ 本文まわりの級数は README §2 の値より一段大きくしてある。
#    実データ（施設名・指標名が長い）で刷ったところ、軸ラベルや注記が
#    画面で読めなかったため。骨格（帯・タイトル・カード見出し・ワードマーク）は
#    正典のまま動かさない。
FS = {
    # ── 骨格：README §2 の値そのまま ─────────────────────────── #
    ...
    # ── 本文まわり：読めるように一段上げた ────────────────────── #
    "slide_meta":   1.15,   # ヘッダー右の補足（正典 1.05）
    "slide_lead":   1.20,   # 本文先頭のリード文（700）
    "body":         1.20,   # 本文テキスト（正典 1.0〜1.2）
    "table_head":   1.05,
    "table_cell":   1.05,   # 表セル（正典 0.88〜1.05）
    "table_cell_s": 0.98,
    "axis_label":   0.95,
    "legend":       1.05,
    "note":         1.05,   # フッタ・欄外の注記（正典 0.95）
}
