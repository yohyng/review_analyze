...
from src import db, review_csv, topic_score  # noqa: E402
...
def build(out: Path) -> dict:
    ...
    for spec in FACILITIES:
        ...
        print(f"  {spec['name']:22s} {ins:4d}件  "
              f"強み={'/'.join(spec['strong']) or '—'}")

    conn.commit()
    return truth


def verify(out: Path, truth: dict) -> int:
    """分析を回して、仕込んだ強み・弱みが再現されるかを確かめる。"""
    ...
    target = FACILITIES[0]["name"]
    peers = [f["name"] for f in FACILITIES[1:6]]
    b = preview.build_bundle(conn, target, matrix, None, None, peers_override=peers)

    print(f"\n{'='*66}\n答え合わせ: {target}\n{'='*66}")
    print(f"較正: {b['score_calibrated']} / 順位 {b['rank']}/{b['total_fac']} "
          f"/ 総合 {(b['overall_sentiment'] or 0)/20:.2f}")

    got_strong = [t for t, _m, _v, _d in b["strengths"][:5]]
    got_weak = [t for t, _m, _v, _d in b["weaknesses"][:5]]
    want_s, want_w = truth[target]["strong"], truth[target]["weak"]

    hit_s = [t for t in want_s if t in got_strong]
    hit_w = [t for t in want_w if t in got_weak]
    print(f"\n仕込んだ強み: {want_s}")
    print(f"検出した強みTOP5: {got_strong}")
    print(f"  → 再現 {len(hit_s)}/{len(want_s)}: {hit_s}")
    print(f"\n仕込んだ弱み: {want_w}")
    print(f"検出した弱みTOP5: {got_weak}")
    print(f"  → 再現 {len(hit_w)}/{len(want_w)}: {hit_w}")

    # 仕込んだ出来事の月を求めて、変化点として拾えているかを確かめる
    start = date.today().replace(day=1) - timedelta(days=30 * MONTHS)
    ev = start + timedelta(days=30 * EVENT_MONTH_OFFSET)
    ev_ym = f"{ev.year}-{ev.month:02d}"
    cps = b["change_points"]
    hit_ev = [c for c in cps if c.ym == ev_ym and c.direction == "down"]
    print(f"\n仕込んだ出来事: {ev_ym} に評価を下げた")
    print(f"検出した変化点: {[(c.ym, c.delta_pt) for c in cps]}")
    print(f"  → {'検出できた' if hit_ev else '検出できず'}")

    ok = len(hit_s) >= 2 and len(hit_w) >= 2 and bool(hit_ev)
    print(f"\n{'='*66}\n判定: {'OK — 仕込んだ特徴を再現できています' if ok else 'NG — 再現できていません'}")
...
def main() -> int:
    ...
    truth = build(out)
    total = sum(f["n"] for f in FACILITIES)
    print(f"\n{len(FACILITIES)}施設 / 口コミ {total}件")
    print(f"\nアプリで使う:\n    VOICEBAUM_DB={out} streamlit run app.py")

    return verify(out, truth) if args.verify else 0
